...
public class WebContainerHandleRequestInterceptor implements AroundInterceptor {
	...
	public WebContainerHandleRequestInterceptor(TraceContext traceContext, MethodDescriptor descriptor,
			RequestRecorderFactory<IRequest> requestRecorderFactory) {
		...
		final ParameterRecorder<IRequest> parameterRecorder = ParameterRecorderFactory
				.newParameterRecorderFactory(config.getExcludeProfileMethodFilter(), config.isTraceRequestParam());

		ServletRequestListenerBuilder<IRequest> builder = new ServletRequestListenerBuilder<>(
				WebsphereConstants.WEBSPHERE, traceContext, requestAdaptor);
	...
	public void before(Object target, Object[] args) {
		...
		try {
			...
			this.servletRequestListener.initialized(request, WebsphereConstants.WEBSPHERE_METHOD,
	...
	private int getStatusCode(final IResponse response) {
		try {
			...
			} else if (response instanceof StoredResponse) {
				StoredResponse r = (StoredResponse) response;
